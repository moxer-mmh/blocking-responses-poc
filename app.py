...
COMPLIANCE_POLICY = {
    ...
    "phi_terms": [
        r"\bdiagnosed\b", r"\bmedical record\b", r"\bmrn\b", r"\bpatient\b",
        r"\bicd-10\b", r"\bcpt\b", r"\bprescription\b", r"\bmedication\b",
        r"\btreatment\b", r"\btherapy\b", r"\bsymptoms\b", r"\bdisease\b",
        r"\bpediatric\b", r"\boncology\b", r"\bpsychiatr(y|ic)\b", r"\bhospital\b",
        r"\bclinic\b", r"\bdoctor\b", r"\bnurse\b", r"\bphysician\b"
    ],
    
    # PCI/Financial contextual terms
    "pci_terms": [
        r"\bcredit card\b", r"\bdebit card\b", r"\bcard number\b", r"\bexpir(y|ation)\b",
        r"\bcvv\b", r"\bsecurity code\b", r"\bpin\b", r"\bbank account\b",
        r"\brouting number\b", r"\biban\b", r"\bswift\b", r"\bpayment\b"
    ],
...
class RegulatedPatternDetector:
    def __init__(self):
        ...
        self.patterns = {
            ...
            "dob": re.compile(r"\b(?:\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s*\d{4})\b", re.I),
            "address": re.compile(r"\b\d{1,5}\s+[A-Za-z0-9.\-\s]+\s+(?:Street|St\.?|Avenue|Ave\.?|Road|Rd\.?|Lane|Ln\.?|Boulevard|Blvd\.?|Drive|Dr\.?|Court|Ct\.?)\b", re.I),
            
            # PCI patterns  
            "credit_card_candidate": re.compile(r"\b(?:\d[ -]*?){13,19}\b"),
            "iban": re.compile(r"\b[A-Z]{2}\d{2}[A-Z0-9]{11,30}\b"),
            "routing_number": re.compile(r"\b\d{9}\b"),
            "bank_account": re.compile(r"\b(?:account\s*number|acct\s*#?)\s*:?\s*\d{6,17}\b", re.I),
            
            # PHI patterns
            "medical_record": re.compile(r"\b(?:mrn|medical\s*record\s*number)\s*:?\s*\d+\b", re.I),
            "diagnosis": re.compile(r"\b(?:diagnosed\s+with|diagnosis\s*:)\s*[a-z\s]+\b", re.I),
            "medication": re.compile(r"\b(?:prescribed|taking|medication)\s+[a-z]+(?:cillin|prazole|statin|mycin)\b", re.I),
            
            # Security patterns
            "password": re.compile(r"\b(?:password|passwd|passphrase)\s*[:=]?\s*\S+\b", re.I),
            "api_key": re.compile(r"\b(?:api[_-]?key|secret[_-]?key|bearer\s+[A-Za-z0-9\._\-]+)\b", re.I),
    ...
    def assess_compliance_risk(self, text: str, region: Optional[str] = None) -> ComplianceResult:
            ...
            if pattern_name == "credit_card_candidate":
                ...
                for match in pattern.finditer(text):
                    if self.luhn_check(match.group(0)):
                        ...
                        triggered_rules.append(f"credit_card: Valid credit card number detected")
                        break
            elif pattern.search(text):
                weight_key = pattern_name.replace("_candidate", "").replace("_context", "_hint")
...
class PresidioDetector:
    ...
    def _add_custom_recognizers(self):
        ...
        cc_recognizer = PatternRecognizer(
            supported_entity="ENHANCED_CREDIT_CARD",
            patterns=[cc_pattern]
        )
...
async def safe_rewrite_stream(user_input: str, detected_types: List[str]) -> AsyncIterator[str]:
    ...
    elif any("credit" in rule or "bank" in rule or "pci" in rule for rule in detected_types):
        template = SAFE_TEMPLATES["pci"]
    elif any("email" in rule or "phone" in rule or "ssn" in rule for rule in detected_types):
        ...
        async def compliance_check_and_stream():
            ...
            try:
                async for piece in upstream_stream(chat_req.message, chat_req.model):
                    ...
                    presidio_score, presidio_entities = presidio_detector.analyze_text(analysis_window)
                    ...
                    if presidio_entities:
                        for entity in presidio_entities:
                            all_triggered_rules.append(f"presidio: {entity['entity_type']} detected (confidence: {entity['score']:.2f})")
                    ...
                    if total_score >= risk_threshold:
                        ...
                        blocked_content_hash = hashlib.sha256(analysis_window.encode()).hexdigest()[:16]
                    ...
                    if (current_tokens > delay_tokens and elapsed_ms >= delay_ms) or elapsed_ms >= delay_ms * 2:
